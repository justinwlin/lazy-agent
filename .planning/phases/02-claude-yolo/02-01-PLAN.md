...
- [ ] Task 1: Create skill directory `.claude/skills/setup-claude-yolo/`
- [ ] Task 2: Create `SKILL.md` with installation and usage guide
- [ ] Task 3: Update `config.example.json` to add `claude_yolo: false` to optional_tools
- [ ] Task 4: Update `config.json` to add `claude_yolo: true` to optional_tools
...
- [ ] `.claude/skills/setup-claude-yolo/SKILL.md` exists and follows skill pattern
- [ ] `config.example.json` includes `claude_yolo` in `optional_tools`
- [ ] `config.json` includes `claude_yolo` in `optional_tools`
